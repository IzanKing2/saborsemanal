-- Publish recipes immediately and remove the admin moderation workflow.
...
SET aprobada = true
WHERE publica = true AND aprobada = false;

ALTER TABLE public.recetas
  ADD CONSTRAINT recetas_publica_requires_aprobada
  CHECK (publica = false OR aprobada = true);

DROP TRIGGER IF EXISTS receta_ingredientes_invalidate_approval
  ON public.receta_ingredientes;
DROP FUNCTION IF EXISTS public.invalidate_recipe_approval();
...
DROP FUNCTION IF EXISTS public.moderate_recipe(UUID, TEXT);
