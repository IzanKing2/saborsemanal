-- Allow recipe authors to use either a master ingredient or a private text label.

ALTER TABLE public.receta_ingredientes
  DROP CONSTRAINT receta_ingredientes_pkey;
...
ALTER TABLE public.receta_ingredientes
  ...
  ADD CONSTRAINT receta_ingredientes_source_check CHECK (
    (ingrediente_id IS NOT NULL AND nombre_personalizado IS NULL)
    OR (
      ingrediente_id IS NULL
      AND length(btrim(nombre_personalizado)) BETWEEN 2 AND 100
    )
  );

CREATE UNIQUE INDEX receta_ingredientes_master_unique_idx
  ON public.receta_ingredientes (receta_id, ingrediente_id)
  WHERE ingrediente_id IS NOT NULL;

CREATE UNIQUE INDEX receta_ingredientes_custom_unique_idx
  ON public.receta_ingredientes (receta_id, lower(btrim(nombre_personalizado)))
...
BEGIN
  ...
  SELECT
    ...
    ingrediente_id,
    CASE
      WHEN ingrediente_id IS NULL THEN btrim(nombre_personalizado)
      ELSE NULL
    END,
    cantidad,
    lower(btrim(unidad))
