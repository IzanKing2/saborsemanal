...
| Ejecutar `add_menu_recipe` | Denegar | Permitir | Permitir en su menú | Denegar |
| Ejecutar `remove_menu_recipe` | Denegar | Permitir | Permitir en su menú | Denegar |
...
- Las RPC son la única vía con privilegios de escritura para `authenticated`.
- Las policies `SELECT` filtran por `auth.uid()`.
- Las migraciones `013`, `014` y `020` coinciden en local y remoto.
- `020` validada en remoto: `save_menu_slot` con "Otro", pool sin asignar
  (`dia_semana`/`tipo_comida` nulos), receta única por semana y smoke test
  `POOL_VALIDATION_OK` (7 comprobaciones).
